...
export class CreatureBuilder extends BuilderBase {
	...
	__getProfBonusInput (cb) {
		...
		const btnAuto = ee`<button class="ve-btn ve-btn-xs ve-btn-default ${this._meta.autoCalc.proficiency ? "active" : ""}" title="Auto-calculate from Challenge Rating (DMG p. 274)"><span class="glyphicon glyphicon-refresh"></span></button>`
			.onn("click", () => {
				if (this._meta.autoCalc.proficiency) {
					this._meta.autoCalc.proficiency = false;
					this.doUiSave();
