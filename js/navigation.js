...
class NavBar {
  ...
  static _initElements() {
    ...
    btnShowHide.onclick = () => {
      ...
      em(`.page__nav-hidden-mobile`).forEach(ele =>
        ele.toggleClass("ve-flex", btnShowHide.classList.contains("active"))
      );
    };
    ...
    this._addElement_dropdown(NavBar._CAT_RULES, NavBar._CAT_BOOKS, {
      isSide: true,
      page: "books.html",
    });
    ...
    this._addElement_dropdown(NavBar._CAT_DUNGEON_MASTER, NavBar._CAT_ADVENTURES, {
      isSide: true,
      page: "adventures.html",
    });
    this._addElement_li(NavBar._CAT_ADVENTURES, "adventures.html", "View All/Homebrew");
    this._addElement_li(
      NavBar._CAT_DUNGEON_MASTER,
      "cultsboons.html",
    ...
    this._addElement_buttonSplit(NavBar._CAT_UTILITIES, {
      metas: [
        {
    ...
    this._addElement_li(
      NavBar._CAT_UTILITIES,
      "https://wiki.tercept.net/en/betteR20",
      "Roll20 Script Help",
      { isExternal: true, isExternalMark: true }
    );
    this._addElement_divider(NavBar._CAT_UTILITIES);
    this._addElement_li(NavBar._CAT_UTILITIES, "changelog.html", "Changelog");
    this._addElement_li(NavBar._CAT_UTILITIES, NavBar._getCurrentWikiHelpPage(), "Page Help", {
      isExternal: true,
      isExternalMark: true,
    });
    this._addElement_divider(NavBar._CAT_UTILITIES);
    ...
    this._addElement_button(NavBar._CAT_SETTINGS, {
      html: "Preferences",
      click: () => {
        ConfigUi.show();
    ...
    this._addElement_button(NavBar._CAT_SETTINGS, {
      ...
      title:
        "Save any locally-stored data (loaded homebrew, active blocklists, DM Screen configuration,...) to a file.",
    });
    this._addElement_button(NavBar._CAT_SETTINGS, {
      html: "Load State from File",
      click: async evt => NavBar.InteractionManager._pOnClick_button_loadStateFile(evt),
      title:
        "Load previously-saved data (loaded homebrew, active blocklists, DM Screen configuration,...) from a file.",
    });
    this._addElement_divider(NavBar._CAT_SETTINGS);
    this._addElement_button(NavBar._CAT_SETTINGS, {
      html: "Add as App",
      ...
      title:
        "Add the site to your home screen. When used in conjunction with the Preload Offline Data option, this can create a functional offline copy of the site.",
    });
    this._addElement_dropdown(NavBar._CAT_SETTINGS, NavBar._CAT_CACHE, { isSide: true });
    this._addElement_label(
      NavBar._CAT_CACHE,
      `<p>Preload data for offline use.</p><p>Note that visiting a page will automatically preload data for that page.</p><p>Note that data which is already preloaded will not be overwritten, unless it is out of date.</p>`
    );
    this._addElement_button(NavBar._CAT_CACHE, {
      html: "Preload Adventure Text <small>(50MB+)</small>",
      click: evt =>
        NavBar.InteractionManager._pOnClick_button_preloadOffline(evt, {
          route: /data\/adventure/,
        }),
    ...
    this._addElement_li(null, "https://wiki.tercept.net/en/5eTools", "Help", {
      isRoot: true,
      isExternal: true,
    });
  }
  ...
  static async _initAdventureBookElements() {
    ...
    const [adventureBookIndex] = await Promise.all([
      DataUtil.loadJSON(
        `${Renderer.get().baseUrl}data/generated/gendata-nav-adventure-book-index.json`
      ),
    ...
    [
      ...
      const metas = [...adventureBookIndex[prop], ...fromPrerelease, ...fromBrew].filter(
        it =>
          !ExcludeUtil.isExcluded(UrlUtil.encodeForHash(it.id), prop, it.source, {
            isNoCount: true,
          })
      );
      ...
      SourceUtil.ADV_BOOK_GROUPS.forEach(({ group, displayName }) => {
        ...
        inGroup.sort(fnSort).forEach(indexMeta => {
          ...
          if (indexMeta.parentSource) {
            if (!this._getNode(indexMeta.parentName)) {
              this._addElement_accordion(parentCategory, indexMeta.parentName, {
            ...
            let cleanName = indexMeta.name.startsWith(indexMeta.parentName)
              ...
              : indexMeta.name;
            this._addElement_li(indexMeta.parentName, page, cleanName, {
              aHash: indexMeta.id,
          ...
          this._addElement_li(parentCategory, page, indexMeta.name, {
            aHash: indexMeta.id,
            date: isNewYear ? year : null,
            isAddDateSpacer: !isNewYear,
  ...
  static _addElement_li(parentCategory, page, aText, opts) {
    ...
    if (opts.isSide) {
      li.onmouseenter = function () {
        NavBar._handleSideItemMouseEnter(this);
      };
    } else {
      li.onmouseenter = function () {
        NavBar._handleItemMouseEnter(this);
      };
      li.onclick = function () {
  ...
  static _addElement_accordion(
    parentCategory,
    category,
    { date = null, isAddDateSpacer = false, source = null } = {}
  ) {
    const parentNode = this._getNode(parentCategory);
    ...
    wrpBody.onclick = function (event) {
  ...
  static _addElement_getDatePrefix({ date, isAddDateSpacer }) {
    return `${date != null || isAddDateSpacer ? `<div class="ve-small mr-2 page__nav-date inline-block ve-text-right inline-block" aria-hidden="true">${date || ""}</div>` : ""}`;
  }
  static _addElement_getSourcePrefix({ source }) {
    return `${source != null ? `<div class="nav2-list__disp-source ${Parser.sourceJsonToSourceClassname(source)}"></div>` : ""}`;
  }
  ...
  static _addElement_dropdown(parentCategory, category, { isSide = false, page = null } = {}) {
    ...
    const li = document.createElement("li");
    li.setAttribute("role", "presentation");
    li.className = `dropdown dropdown-${category.split(" ").join("-")} dropdown--navbar page__nav-hidden-mobile ${isSide ? "" : "page__btn-nav-root"}`;
    if (isSide) {
      li.onmouseenter = function () {
        NavBar._handleSideItemMouseEnter(this);
      };
  ...
  a.setAttribute("aria-haspopup", "true");
    a.onclick = function (event) {
      NavBar._handleDropdownClick(this, event, isSide,`dropdown-${category.split(" ").join("-")}`);
    };
    if (isSide) {
      a.onmouseenter = function () {
        NavBar._handleSideDropdownMouseEnter(this);
      };
      a.onmouseleave = function () {
        NavBar._handleSideDropdownMouseLeave(this);
      };
  ...
  static _addElement_buttonSplit(parentCategory, options) {
    ...
    options.metas.forEach(({ className, click, html, title }, i) => {
      const eleSpan = document.createElement("span");

      eleSpan.className = [className, "inline-block", i ? null : "w-100 min-w-0"]
        .filter(Boolean)
        .join(" ");
  ...
  static highlightCurrentPage() {
    ...
    if (typeof _SEO_PAGE !== "undefined") currentPage = `${_SEO_PAGE}.html`;
      ...
      currentPage.toLowerCase() === "book.html" ||
      currentPage.toLowerCase() === "adventure.html"
    ) {
  ...
  static _handleDropdownClick(dropdown, event, isSide, dropdownClass) {
    event.preventDefault()
    event.stopPropagation();

    const isOpen = this._isDropdownOpen(dropdown);
    if (isOpen) NavBar._dropdowns.forEach(ele =>
		{
			if(ele.classList.contains(dropdownClass)) 
				NavBar._closeDropdownElement(ele)
		}
	);
    else NavBar._openDropdown(dropdown);
  }
  ...
  static _openDropdown(ele) {
    ...
    NavBar._dropdowns
      .filter(ele => !lisOpen.includes(ele))
      .forEach(ele => NavBar._closeDropdownElement(ele));
  ...
  static _openDropdown_mutAlignment({ liNavbar }) {
    ...
    const isForceRightAlign =
      liNavbar.getBoundingClientRect().left >= window.innerWidth - widthRequired;

    uls.forEach(ul => {
      ul.style.left = isForceRightAlign
        ? `${-Math.round(ul.getBoundingClientRect().width) + ul.parentNode.getBoundingClientRect().width}px`
        : "";
    });
  }
  ...
  static _handleItemMouseEnter(ele) {
    ele = e_(ele);
    const timerIds = ele
      .siblings("[data-timer-id]")
      .map(eleSib => ({ ele: e_(eleSib), timerId: e_(eleSib).attr("data-timer-id") }));
        ...
        const getTimeoutFn = () => {
          if (NavBar._timerMousePos[timerId]) {
            ...
            if (EventUtil._mouseX > xStart + NavBar._MIN_MOVE_PX) {
              ...
              NavBar._timersClose[timerId] = setTimeout(
                () => getTimeoutFn(),
                NavBar._DROP_TIME / 2
              );
            } else {
