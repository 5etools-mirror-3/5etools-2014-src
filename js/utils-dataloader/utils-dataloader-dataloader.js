...
class DataTypeLoaderCharacter extends DataTypeLoader {
	...
	async _pGetSiteData ({pageClean, sourceClean}) {
		...
		if (!CharacterManager) return {character: []};
	...
	async _pGetStoredPrereleaseData () { return null; }

	async _pGetStoredBrewData () { return null; }
}
...
export class DataLoader {
	...
	static _registerDataTypeLoaders () {
		...
		DataTypeLoaderCharacter.register({fnRegister});
		// endregion
	...
	static async pCacheAndGet (
		...
		if (pageClean === "characters" && hashClean && globalThis.CharacterManager) {
			try {
				const CharacterManager = globalThis.CharacterManager;
				const possibleIds = [
					hashClean,
					hashClean.replace("_", ""),
					sourceClean,
					`${sourceClean}_${hashClean.split("_")[0]}`,
					hashClean.split("_")[0],
				];
				...
				if (!character) {
					try {
						const summaries = await CharacterManager.loadCharacterSummaries();
						const characterName = hashClean.split("_")[0];
				...
				console.warn(`Failed to lazy load character with hash ${hashClean}:`, e);
			}
		}
